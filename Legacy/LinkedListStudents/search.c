...
int compareKey(key k1, key k2) {
  ...
  if (k1.roll > k2.roll)
    ...
    ret_value =
        strcmp(k1.sub_code,
               k2.sub_code); // the compare value will depend on sub code only
  }
  return ret_value;
  // 1 for k1>k2
  //-1 for k1<k2
...
find searchKey(node *head, node **prev, node **current, key k) {
  ...
  {
    if (compareKey(k, cur->s.k) ==
        0) // for checking if present in list and then stop
    {
      f = FOUND;
      flag = 1;
    } else if (compareKey(k, cur->s.k) ==
               -1) // if key is less than current node key then stop
    {
      flag = 1;
    } else // increament
    {
