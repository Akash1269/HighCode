...
{
     node *ptr=head,*max_list=NULL;// max_list is head to the list of student
who hav max marks int max_marks=0; while(ptr!=NULL)
     {
         if(strcmp(sub_code,ptr->s.k.sub_code)==0)
         {
                ...
                else if(max_marks==ptr->s.marks)//if the found max is the only
max at this step
                {
                    node *nptr;
...
node *getMaxMarks(char sub_code[], node *head) {
    node *ptr = head, *max_list = NULL,
         *first_max = head; // max_list is head to the list of student who hav max marks
    int max_marks = 0;
